      ...
      <ContainerLayout>
        ...
        <div className="mt-10 grid gap-10 sm:grid-cols-3">
          {teamMembers.map((member, idx) => (
            <div
              key={idx}
              className="group relative rounded-xl bg-white/5 backdrop-blur-sm p-6 text-center shadow-md hover:shadow-purple-500/20 transition duration-300"
            >
              <div className="relative w-28 h-28 mx-auto mb-4">
                <Image
                  ...
                  alt={member.name}
                  fill
                  className="rounded-full object-cover border-2 border-purple-400/40 shadow-lg"
                />
              </div>
              ...
              <div className="mt-4 flex justify-center gap-4 text-purple-300">
                {member.socials.linkedin && (
                  <a href={member.socials.linkedin} target="_blank" rel="noreferrer">
                    <FaLinkedin className="size-6 hover:text-white transition" />
                  </a>
                )}
                {member.socials.x && (
                  <a href={member.socials.x} target="_blank" rel="noreferrer">
                    <FaSquareXTwitter className="size-6 hover:text-white transition" />
                  </a>
                )}
                {member.socials.github && (
                  <a href={member.socials.github} target="_blank" rel="noreferrer">
                    <FaSquareGithub className="size-6 hover:text-white transition" />
                  </a>
                )}
              </div>
            </div>
