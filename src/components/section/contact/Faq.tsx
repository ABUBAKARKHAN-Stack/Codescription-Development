...
export default function FAQSection() {
  ...
  return (
    <section className="mx-auto bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 px-4 py-12">
      <ContainerLayout>
        ...
        <div className="mt-10 space-y-4">
          {faqs.map((faq, index) => (
            ...
            >
              <button
                className={`flex w-full justify-between ${openIndex === index ? "bg-white/10 text-white hover:bg-white/10" : "cursor-pointer group-hover:bg-gray-50 group-hover:text-black"} items-center px-6 py-4 text-left text-lg font-medium transition-colors`}
              ...
              <AnimatePresence initial={false}>
                {openIndex === index && (
                  ...
                  >
                    <div
                      className={`${openIndex === index ? "bg-white" : null} px-6 py-4 text-gray-600`}
                    >
                      {faq.answer}
                    </div>
                  </motion.div>
